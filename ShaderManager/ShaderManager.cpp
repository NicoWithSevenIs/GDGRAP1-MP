...
void ShaderManager::LoadSkyboxShader(std::string path, unsigned int shader_type) {
    ...
    if (!this->skyboxShader) {
        this->skyboxShader = new GLuint(glCreateProgram());
    }

    glAttachShader(*this->skyboxShader, shader);

}

GLuint* ShaderManager::getShaderProg() { return this->shaderProg; }
GLuint* ShaderManager::getSkyboxShader() { return this->skyboxShader; }